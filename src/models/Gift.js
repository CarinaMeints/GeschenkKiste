...
giftSchema.statics.recommendForEvent = async function (
  ...
  const GiftInterestLink = mongoose.model("GiftInterestLink");
  ...
  const linkDocs = await GiftInterestLink.find({
    gift: { $in: candidateIds },
    $or: [{ isPublic: true }, { createdBy: userId }],
  })
    .select("gift interest")
    .lean();

  const linkInterestIdsByGiftId = new Map();
  for (const l of linkDocs) {
    const gid = String(l.gift);
    const iid = String(l.interest);
    if (!gid || !iid) continue;
    if (!linkInterestIdsByGiftId.has(gid))
      linkInterestIdsByGiftId.set(gid, new Set());
    linkInterestIdsByGiftId.get(gid).add(iid);
  }
  ...
  const scored = candidates.map((gift) => {
    const baseIds = (gift.interests || []).map((i) => String(i?._id || i));
    const extraSet = linkInterestIdsByGiftId.get(String(gift._id)) || new Set();

    const giftInterestIds = Array.from(
      new Set([...baseIds, ...Array.from(extraSet)]),
    );
    ...
    if (matchPoints > 0) score += usageEff * USAGE_PER_GIFT;
