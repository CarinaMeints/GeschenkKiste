...
const EventController = {
  ...
  async show(req, res) {
    try {
      ...
      const assignedGiftIdsForThisEvent = new Set(
        (assignmentsRaw || [])
          .map((a) =>
            a?.gift?._id ? String(a.gift._id) : a?.gift ? String(a.gift) : "",
          )
          .filter(Boolean),
      );

      const giftedGiftIdsForPerson = new Set(
        (giftHistory || [])
          .filter((a) => String(a?.status || "").toLowerCase() === "fertig")
          .map((a) =>
            a?.gift?._id ? String(a.gift._id) : a?.gift ? String(a.gift) : "",
          )
          .filter(Boolean),
      ...
      const availableGiftsFiltered = (availableGifts || []).filter((g) => {
        const gid = String(g?._id || "");
        if (!gid) return false;

        if (assignedGiftIdsForThisEvent.has(gid)) return false;
        if (giftedGiftIdsForPerson.has(gid)) return false;

        return true;
      });

      const suggestionsFiltered = (suggestions || []).filter((g) => {
        const gid = String(g?._id || "");
        if (!gid) return false;
        if (assignedGiftIdsForThisEvent.has(gid)) return false;
        if (giftedGiftIdsForPerson.has(gid)) return false;
      ...
      return res.render("events/show", {
        ...
        availableGifts: availableGiftsFiltered,
        availableInterests,
        interestMeta,
        giftHistory,
        giftState,
