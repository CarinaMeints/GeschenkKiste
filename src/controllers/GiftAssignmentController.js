...
const GiftAssignmentController = {
  async create(req, res) {
    try {
      ...
      const existsForEvent = await GiftAssignment.findOne({
        gift: gift._id,
        event: event._id,
        createdBy: userId,
      })
      ...
      if (existsForEvent) {
        req.session.error =
          "Dieses Geschenk ist diesem Event bereits zugeordnet.";
        return redirectToEvent(res, event._id, returnTo);
      }

      const personId = event?.personOccasion?.person?._id;
      const personName = event?.personOccasion?.person?.name || "diese Person";

      if (personId) {
        const giftedHistory = await GiftAssignment.findGiftedForPerson(
          personId,
          userId,
        );

        const alreadyGifted = (giftedHistory || []).some((a) => {
          const g = a?.gift?._id ? String(a.gift._id) : String(a.gift);
          const st = String(a?.status || "").toLowerCase();
          return g === String(gift._id) && st === "fertig";
        });

        if (alreadyGifted) {
          req.session.error = `Dieses Geschenk wurde ${personName} bereits geschenkt.`;
          return redirectToEvent(res, event._id, returnTo);
        }
      }
