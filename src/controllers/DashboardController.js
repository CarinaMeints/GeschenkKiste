...
const DashboardController = {
  async index(req, res) {
    try {
      ...
      const dateTasksOpenRaw = await Task.find({
        createdBy: userIdRaw,
        isDone: false,
      ...
      const dateTasksOpen = sortTasksChronologically(dateTasksOpenRaw);
      const dateTasksPreview = dateTasksOpen.slice(0, 5);

      const dateTasksMoreCount = Math.max(0, dateTasksOpen.length - 5);
      const dateTasksMorePreview = dateTasksOpen.slice(5, 30);
      const dateTasksMoreHiddenCount = Math.max(
        0,
        dateTasksOpen.length - 5 - dateTasksMorePreview.length,
      );
      ...
      const eventsSortedForPreview = [...filteredForPreview].sort((a, b) => {
      ...
      const eventsPreview = eventsSortedForPreview.slice(0, 8);

      const eventsMoreCount = Math.max(0, eventsSortedForPreview.length - 8);
      const eventsMorePreview = eventsSortedForPreview.slice(8, 30);
      const eventsMoreHiddenCount = Math.max(
        0,
        eventsSortedForPreview.length - 8 - eventsMorePreview.length,
      );
      ...
      return res.render("dashboard", {
        ...
        dateTasksMoreCount,
        dateTasksMorePreview,
        dateTasksMoreHiddenCount,
        eventsPreview,
        eventsMoreCount,
        eventsMorePreview,
        eventsMoreHiddenCount,
        eventFilter,
        eventCounts,
      });
    } catch (err) {
      ...
      return res.render("dashboard", {
        ...
        dateTasksPreview: [],
        dateTasksCount: 0,
        dateTasksMoreCount: 0,
        dateTasksMorePreview: [],
        dateTasksMoreHiddenCount: 0,
        eventsPreview: [],
        eventsMoreCount: 0,
        eventsMorePreview: [],
        eventsMoreHiddenCount: 0,
        eventFilter: "all",
