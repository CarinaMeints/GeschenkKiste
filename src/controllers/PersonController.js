...
function parseISODateStrict(v) {
  ...
  return { date: d, year: yy, month: mm, day: dd };
}

function validateFixedDayMonth(day, month) {
  const hasDay = day != null && String(day).trim() !== "";
  const hasMonth = month != null && String(month).trim() !== "";

  if (hasDay !== hasMonth) {
    return "Bitte Tag und Monat gemeinsam ausfüllen (oder beide leer lassen).";
  }
  ...
  if (
    ...
    return "Ungültiges Datum: Bitte Tag (1–31) und Monat (1–12) korrekt angeben.";
  }

  if (!isValidDayMonth(d, m, { allowFeb29: true })) {
    return "Ungültiges Datum: Dieser Tag existiert in diesem Monat nicht (z.B. 31. Februar).";
  }
...
const PersonController = {
  ...
  async create(req, res) {
    ...
    try {
      ...
      if (newOccasionName && newOccasionName.trim()) {
        const fixedErr = validateFixedDayMonth(
          newOccasionDay,
          newOccasionMonth,
        );
        if (fixedErr) return renderNewWithError(fixedErr);
      ...
      for (const occasionIdRaw of occasionIds) {
        ...
        if (occasion.isRecurring === false) endYear = startYear;
        ...
        if (customDateStr) {
          const parsed = parseISODateStrict(customDateStr);
          if (!parsed) {
            return renderNewWithError(
              "Ungültiges Datum. Bitte ein gültiges Datum auswählen.",
            );
          }

          const year = parsed.year;

          if (year < YEAR_MIN || year > YEAR_MAX_MOVABLE) {
            return renderNewWithError(
              `Ungültiges Jahr (${year}). Bitte ein Jahr zwischen ${YEAR_MIN} und ${YEAR_MAX_MOVABLE} wählen.`,
            );
          }
          ...
          if (isMovableOccasion(occasion)) {
            ...
            link.customMonth = parsed.month;
            link.customDay = parsed.day;
            await link.save();
          }

          await Event.findOrCreate(link._id, year, parsed.date, userId);
  ...
  async update(req, res) {
    try {
      ...
      person.name = name;
      person.birthdayDay = bd;
      person.birthdayMonth = bm;
      person.notes = notes;
      ...
      async function applyCustomDateIfProvided({ link, occasion, dateStr }) {
        if (!isMovableOccasion(occasion)) return;
        ...
        if (year < link.startYear) {
          throw new Error(`Datum liegt vor dem Startjahr (${link.startYear}).`);
        }
        if (link.endYear != null && year > link.endYear) {
          throw new Error(`Datum liegt nach dem Endjahr (${link.endYear}).`);
        }
      ...
      for (const oid of toAdd) {
        ...
        if (occ.isRecurring === false) endYear = startYear;
        ...
        const customDateStr = customDates[String(oid)];
        await applyCustomDateIfProvided({
          link,
          occasion: occ,
      ...
      for (const oid of selectedOccasionIds) {
        ...
        if (occ.isRecurring === false) parsedEndYear = parsedStartYear;
      ...
      console.error(err);
      req.session.error = err?.message || "Fehler beim Aktualisieren";
      return res.redirect(`/persons/${req.params.id}/edit`);
    }
  },
  ...
  async assignOccasion(req, res) {
    try {
      ...
      if (customDate) {
        const parsed = parseISODateStrict(customDate);
        if (!parsed) {
          req.session.error =
            "Ungültiges Datum. Bitte ein gültiges Datum auswählen.";
          return res.redirect(`/persons/${req.params.id}/occasions`);
        ...
        if (isMovableOccasion(occasion)) {
          if (year < YEAR_MIN || year > YEAR_MAX_MOVABLE) {
            req.session.error = `Ungültiges Jahr (${year}). Bitte ein Jahr zwischen ${YEAR_MIN} und ${YEAR_MAX_MOVABLE} wählen.`;
            return res.redirect(`/persons/${req.params.id}/occasions`);
          }
        } else {
          if (year < 1900 || year > 9999) {
            req.session.error =
              "Ungültiges Jahr. Erlaubt sind Jahre von 1900 bis 9999.";
            return res.redirect(`/persons/${req.params.id}/occasions`);
          }
        }
      ...
      console.error(err);
      req.session.error = err?.message || "Fehler beim Zuordnen";
      return res.redirect(`/persons/${req.params.id}/occasions`);
    }
  },
  ...
  async updatePersonOccasion(req, res) {
    try {
      ...
      if (link.occasion?.isRecurring === false) endYear = startYear;
  ...
  async updateOccasionDate(req, res) {
    ...
    try {
      ...
      if (req.query.year) {
        ...
        if (
          !requestedYear ||
          requestedYear < YEAR_MIN ||
          requestedYear > YEAR_MAX_MOVABLE
        ) {
          req.session.error = "Ungültiges Jahr in der Anfrage.";
          return res.redirect(backToEditUrl);
        }
        if (year !== requestedYear) {
          req.session.error = `Bitte ein Datum im Jahr ${requestedYear} auswählen.`;
          return res.redirect(backToEditUrl);
        }
      }
  ...
  async addHistoryGift(req, res) {
    try {
      ...
      if (newGiftTitle) {
        safeNewGiftLink = normalizeHttpUrl(newGiftLink);
      ...
      if (occasionHasFixedDate) {
        ...
        if (ddd !== occasion.day || mmm !== occasion.month) {
      ...
      if (occasion.isBirthday === true) {
        ...
        const ddd = date.getDate();
        const mmm = date.getMonth() + 1;
        if (ddd !== person.birthdayDay || mmm !== person.birthdayMonth) {
      ...
      req.session.success = `Historie hinzugefügt${
        createdGiftTitle ? ` · Geschenk: ${createdGiftTitle}` : ""
      }`;
      return res.redirect(`/persons/${personId}#history`);
    } catch (err) {
      console.error(err);
      req.session.error = err?.message || "Fehler beim Hinzufügen zur Historie";
      return res.redirect(`/persons/${req.params.id}#history`);
    }
  },
