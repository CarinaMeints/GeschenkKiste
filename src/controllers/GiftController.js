...
const {
  mergeGiftInterestsForUser,
} = require("../lib/mergeGiftInterestsForUser");
...
const GiftController = {
  async index(req, res) {
    try {
      ...
      await mergeGiftInterestsForUser(giftsRaw, userId);
  ...
  async show(req, res) {
    try {
      ...
      await mergeGiftInterestsForUser(gift, userId);
  ...
  async update(req, res) {
    ...
    uploadMiddleware(req, res, async (err) => {
      ...
      try {
        ...
        console.error(err2);
        cleanupUploadedFiles();
        req.session.error = err2.message || "Fehler beim Speichern";
        return res.redirect(`/gifts/${req.params.id}/edit`);
      }
    });
  },
