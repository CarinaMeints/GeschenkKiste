...
const OccasionController = {
  ...
  async create(req, res) {
    try {
      ...
      const hasDay = !!dayStr;
      const hasMonth = !!monthStr;

      if (hasDay !== hasMonth) {
        req.session.error =
          "Bitte Tag und Monat gemeinsam ausfüllen (oder beide leer lassen).";
      ...
      if (hasDay && hasMonth) {
        ...
        if (
          ...
          req.session.error =
            "Ungültiges Datum: Bitte Tag (1–31) und Monat (1–12) korrekt angeben.";
          return res.redirect("/occasions/new");
        }

        if (!isValidDayMonth(d, m, { allowFeb29: true })) {
          req.session.error =
            "Ungültiges Datum: Dieser Tag existiert in diesem Monat nicht (z.B. 31. Februar).";
          return res.redirect("/occasions/new");
        }
      }
      ...
      const doc = await Occasion.create({
        name,
        description,
        day: hasDay ? parseInt(dayStr, 10) : undefined,
        month: hasMonth ? parseInt(monthStr, 10) : undefined,
  ...
  async update(req, res) {
    try {
      ...
      occasion.day = hasDay ? parseInt(dayStr, 10) : undefined;
      occasion.month = hasMonth ? parseInt(monthStr, 10) : undefined;
  ...
  async destroy(req, res) {
    try {
      ...
      const dep = await PersonOccasion.deleteWithDependents({
        userId,
        filter: { occasion: occasion._id },
      });
      ...
      req.session.success =
        `Anlass gelöscht` +
        (dep?.linksDeleted
          ? ` (inkl. ${dep.linksDeleted} Personen-Zuordnung(en), ${dep.eventsDeleted} Event(s), ${dep.assignmentsDeleted} Geschenk-Zuordnung(en), ${dep.tasksDeleted} Task(s))`
          : "");
