(() => {
  ...
  function onOccasionCheckboxChange(e) {
    ...
    toggleById(cb.dataset.dateTarget, cb.checked && isMovable);
  }

  document.addEventListener("click", onPickInterestClick);
  document.addEventListener("change", onOccasionCheckboxChange);

  function initOccasionCheckboxes() {
    document.querySelectorAll("input.occ-checkbox").forEach((cb) => {
      const isMovable = cb.dataset.movable === "true";
      toggleById(cb.dataset.rangeTarget, cb.checked);
      toggleById(cb.dataset.dateTarget, cb.checked && isMovable);
    });
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", initOccasionCheckboxes);
  } else {
    initOccasionCheckboxes();
  }
})();
